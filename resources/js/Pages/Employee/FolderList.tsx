...
import FolderModuleCard, { type OrientationFolder } from './components/folder/FolderModuleCard'
...
export default function FolderList({ folders, allCompleted }: Props) {
    ...
    const progress = folders.length ? (completedCount / folders.length) * 100 : 0
        ...
        <>
            ...
            <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-6 py-10 font-poppins">
                ...
                <div className="mb-8 space-y-4">
                    <div>
                        <h1 className="text-2xl font-semibold tracking-tight">Orientation Modules</h1>
                        <p className="mt-1 text-sm text-muted-foreground">
                            Complete each module in order. Finish all {folders.length} to move on to your final
                            acknowledgement.
                        </p>
                    </div>
                ...
                <div className="grid flex-1 grid-cols-1 gap-4 content-start sm:grid-cols-2 lg:grid-cols-3">
                    {folders.map((folder, index) => (
                        <FolderModuleCard key={folder.id} folder={folder} index={index} />
                    ))}
                </div>
                ...
                {allCompleted && (
                    <div className="mt-8 flex flex-col items-center gap-3 rounded-2xl border bg-muted/30 p-6 text-center">
                        <p className="text-sm font-medium">
                            You've completed all modules. You're ready for the final step.
                        </p>
